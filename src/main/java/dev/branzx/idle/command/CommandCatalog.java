...
public final class CommandCatalog {
    ...
    private static final List<Entry> PLAYER_BASE = List.of(
            ...
            player("rotate", "", "หมุนสิ่งปลูกสร้างของ Node ปัจจุบัน", "territory"),
            player("building", "[id]", "เลือกสกินสิ่งปลูกสร้างของ Node ปัจจุบัน", "territory"),
            player("submit", "[start|spawn <slot>|work <slot>|done <name>|cancel]",
                    "ส่งสิ่งปลูกสร้างที่สร้างเองเข้าประกวดสกิน", "territory"),
            player("merge", "[shape]", "รวม Residential รอบๆ เป็นสิ่งปลูกสร้างผืนใหญ่", "territory"),
            player("unmerge", "", "แยก Complex กลับเป็น Node เดี่ยว", "territory"),
            ...
            restrictedPlayer("skin", "<name>", "เปลี่ยนสกิน Worker", "workers", "idle.skin"),
    ...
    private static final List<Entry> ADMIN = List.of(
            admin("dashboard", "", "เปิด Admin Hub", "overview", "idle.admin"),
            admin("node", "", "ตรวจสอบ Node ที่ยืนอยู่", "inspection", "idle.admin.operations"),
            admin("claims", "<player>", "ดู claim ของผู้เล่น", "inspection", "idle.admin.operations"),
            admin("audit", "[player]", "ดู audit log ล่าสุด", "inspection", "idle.admin.audit"),
            admin("metrics", "", "ดู economy/telemetry summary", "system", "idle.admin.audit"),
            admin("validate", "", "ตรวจสอบ content ก่อน publish", "system", "idle.admin.content"),
            admin("reload", "<reason>", "โหลด config, pools และ schematics ใหม่", "system", "idle.admin.operations"),
            admin("pool", "[type[.bracket-N]|publish|rollback|discard] [reason]",
                    "เปิด/จัดการ drop pool draft", "content", "idle.admin.content"),
            admin("schem", "<action> ...", "เครื่องมือสร้าง schematic", "content", "idle.admin.content"),
            admin("npc", "<refresh|list|state> ... [reason]", "ตรวจสอบและ refresh Worker NPC", "content", "idle.admin.content"),
            admin("event", "<spawn|cancel|list> [type] [reason]", "จัดการ Exploration event", "operations", "idle.admin.operations"),
            admin("explevel", "<level> <reason>", "ตั้ง Exploration level ของ Node", "operations", "idle.admin.operations"),
            admin("setcap", "<player> <base> <bonus> <reason>", "ตั้ง Node cap", "economy", "idle.admin.economy"),
            admin("skingrant", "<player> <grant|revoke> <skinId> <reason>",
                    "ให้/ถอนสกินสิ่งปลูกสร้าง", "content", "idle.admin.content"),
            admin("skinreview", "[list|show|spawn|work|approve|reject] <id> ...",
                    "รีวิวผลงานที่ผู้เล่นส่งเข้าประกวด", "content", "idle.admin.content"),
            admin("give", "<money|item> ... <reason>", "ปรับ Coins หรือมอบ item", "economy", "idle.admin.economy"),
            admin("credits", "<player> <amount> <reason>", "ปรับ Credits", "economy", "idle.admin.economy"),
            admin("forceunclaim", "<world> <x> <z> <reason>", "บังคับยกเลิก claim", "danger", "idle.admin.operations")
    );
