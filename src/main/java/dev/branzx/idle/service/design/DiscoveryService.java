...
public final class DiscoveryService {
    ...
    public DiscoveryService(IdlePlugin plugin, Database database, TelemetryService telemetry) {
        this.plugin = plugin;
    ...
    public void loadSync() {
        try (Connection connection = database.getConnection()) {
            try (PreparedStatement select = connection.prepareStatement(
                    "SELECT owner_uuid, node_type, material, lifetime_count FROM idle_discoveries");
            ...
            try (PreparedStatement select = connection.prepareStatement(
                    "SELECT owner_uuid, material, period_key, amount FROM idle_resource_caps");
                 ResultSet rs = select.executeQuery()) {
                while (rs.next()) {
        ...
        database.submitWrite(() -> {
            try (Connection connection = database.getConnection()) {
                ...
                try (PreparedStatement update = connection.prepareStatement(
                        "UPDATE idle_discoveries SET lifetime_count = ? "
        ...
        database.submitWrite(() -> {
            try (Connection connection = database.getConnection();
                 PreparedStatement upsert = connection.prepareStatement(
                         "REPLACE INTO idle_resource_caps (owner_uuid, material, period_key, amount) "
                                 + "VALUES (?, ?, ?, ?)")) {
