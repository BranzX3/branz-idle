        ...
        try (Connection connection = database.getConnection()) {
            try (PreparedStatement query = connection.prepareStatement(
                    "SELECT COUNT(*) AS players, COALESCE(SUM(balance), 0) AS total, "
                            + "COALESCE(MAX(balance), 0) AS richest FROM idlefarm_players");
            ...
            try (PreparedStatement query = connection.prepareStatement(
                    "SELECT COUNT(*) AS total FROM idlefarm_nodes WHERE node_type <> 'RESIDENTIAL'");
                 ResultSet rs = query.executeQuery()) {
                if (rs.next()) nodes = rs.getLong("total");
            }
            try (PreparedStatement query = connection.prepareStatement(
                    "SELECT COALESCE(SUM(capacity), 0) AS total FROM idlefarm_warehouse");
                 ResultSet rs = query.executeQuery()) {
                if (rs.next()) capacity = rs.getLong("total");
            }
            try (PreparedStatement query = connection.prepareStatement(
                    "SELECT event_type, detail_json FROM idlefarm_telemetry "
